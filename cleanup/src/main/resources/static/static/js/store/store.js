		...
		for(var i=0;i<result.length;i++) {
			...
			if(store.isOpen == "false") {
				isOpenHtml = `<div class="is_open">
								<a href="/store/detail/${id }">영업 시간 종료</a>
							</div>`;
			}
				...
				<div class="info_box">
					...
					<a href="/store/detail/${id }">
						...
						<span>
							...
							<span>세탁소 답변 ${bossCommentCount }</span>
						</span>
						
						<span>
							<span>기본의뢰뢰금액 ${minDelevery }원</span>
							<span>세탁소 할인 이벤트 ${deleveryTip }원</span>
						</span>
						<span>배송시간 ${deleveryTime }일</span>
					</a>
				</div>
        	${isOpenHtml}
        </li>`;
