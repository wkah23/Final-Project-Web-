$(document).ready(function() {
	...
	$(".add_cart").click(function(){
		...
		if(cartStoreId != null && storeId != cartStoreId ) {
			swal({
				buttons: ["취소", "담기"],
				title: "세탁바구니에는 같은 세탁소의 서비스만 담을 수 있습니다",
				text: "선택하신 서비스를 세탁바구니에 담을 경우 이전에 담은 서비스가 삭제됩니다"
			})
			.then((value) => {
				if (value == true) {
					deleteCartAll();
	...
	function cartList(result){
		...
		console.log("세탁바구니 가게 id " + storeId);
	...
	function cartReset() {
		$(".cart ul").html("");
		$(".total").html("세탁바구니가 비었습니다.");
		$(".order_btn").css("background", "#rgb(255, 255, 255)");
		$(".order_btn").attr("disabled", true); 
		$(".order_btn").text("의뢰하기");
